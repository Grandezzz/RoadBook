...
let authSessionTemporary = false;
const authStorage = {
  async getItem(key){try{const value=sessionStorage.getItem(key);if(value!==null)return value;}catch{}try{return localStorage.getItem(key);}catch{return null;}},
  async setItem(key,value){try{localStorage.setItem(key,value);try{sessionStorage.removeItem(key);}catch{}authSessionTemporary=false;}catch(localError){try{sessionStorage.setItem(key,value);authSessionTemporary=true;}catch{throw localError;}}},
  async removeItem(key){try{localStorage.removeItem(key);}catch{}try{sessionStorage.removeItem(key);}catch{}}
};
const cloud = supabaseReady ? window.supabase.createClient(cloudConfig.url, cloudConfig.anonKey, {auth:{storage:authStorage}}) : null;
let cloudUser = null, cloudTimer = null, authBusy = false, accessNotice = '';
let imageSaveBusy = false;
const imageUrls = new Map(), imageUrlRequests = new Map(), previewUrls = new Map();
const previewJobs = new Set(), warmedImages = new Set();
let passwordLinkType = new URLSearchParams(window.location.hash.slice(1)).get('type');
let inviteNeedsPassword = ['invite','recovery'].includes(passwordLinkType);
...
function save(){if(cloudUser){scheduleCloudSave();return;}localStorage.setItem(KEY,JSON.stringify(state));}
function load(){ try { state = JSON.parse(localStorage.getItem(KEY)) || state; } catch { /* use empty state */ } }
function embeddedImages(data){return data.roadbooks.some(book=>Object.values(book.days||{}).flat().some(item=>imageList(item).some(image=>typeof image==='string'&&image.startsWith('data:'))));}
function imageList(item){return Array.isArray(item.images)?item.images:typeof item.images==='string'&&item.images?[item.images]:[];}
async function persistCloudState(data){if(embeddedImages(data))throw new Error('旧图片尚未迁移到云端，无法保存');const {error}=await cloud.from('roadbook_data').upsert({user_id:cloudUser.id,payload:data,updated_at:new Date().toISOString()});if(error)throw error;}
function scheduleCloudSave(){clearTimeout(cloudTimer);cloudTimer=setTimeout(async()=>{if(!cloudUser)return;try{await persistCloudState(state);}catch(error){showToast('云端保存失败：'+error.message);}},700);}
function imageExtension(type){return ({'image/jpeg':'jpg','image/png':'png','image/webp':'webp','image/gif':'gif','image/heic':'heic','image/heif':'heif'})[type]||null;}
async function resizeImage(blob,maxEdge=1600){if(typeof createImageBitmap!=='function'||!['image/jpeg','image/png','image/webp'].includes(blob.type))return blob;let bitmap;try{bitmap=await createImageBitmap(blob);const scale=Math.min(1,maxEdge/Math.max(bitmap.width,bitmap.height));if(scale===1&&blob.size<350_000)return blob;const canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(bitmap.width*scale));canvas.height=Math.max(1,Math.round(bitmap.height*scale));canvas.getContext('2d').drawImage(bitmap,0,0,canvas.width,canvas.height);const optimized=await new Promise(resolve=>canvas.toBlob(resolve,'image/webp',0.78));return optimized&&optimized.type==='image/webp'&&optimized.size<blob.size?optimized:blob;}catch{return blob;}finally{bitmap?.close?.();}}
async function uploadImage(file){if(!cloudUser)throw new Error('请先登录');if(!imageExtension(file.type))throw new Error('图片格式不受支持');const body=await resizeImage(file);const ext=imageExtension(body.type);const path=`${cloudUser.id}/${uid()}.${ext}`;const {error}=await cloud.storage.from(IMAGE_BUCKET).upload(path,body,{contentType:body.type,upsert:false,cacheControl:'3600'});if(error)throw new Error('图片上传失败：'+error.message);return path;}
async function migrateEmbeddedImages(){if(!embeddedImages(state))return;const next=structuredClone(state);let count=0;for(const book of next.roadbooks){for(const items of Object.values(book.days||{})){for(const item of items){const originals=imageList(item);if(!originals.some(image=>typeof image==='string'&&image.startsWith('data:')))continue;item.images=[];for(const image of originals){if(typeof image==='string'&&image.startsWith('data:')){const blob=await (await fetch(image)).blob();item.images.push(await uploadImage(blob));count++;}else item.images.push(image);}}}}await persistCloudState(next);state=next;render();showToast(`已将 ${count} 张旧图片迁移到云端`);}
function clearMatchingLocalCopies(original){let different=false;for(const key of [KEY,LOCAL_BACKUP_KEY]){try{const raw=localStorage.getItem(key);if(!raw)continue;const copy=JSON.parse(raw);if(JSON.stringify(copy)===JSON.stringify(original))localStorage.removeItem(key);else if(copy?.roadbooks?.length)different=true;}catch{different=true;}}return different;}
async function signedImageUrl(path){const cached=imageUrls.get(path);if(cached&&cached.expires>Date.now())return cached.url;if(imageUrlRequests.has(path))return imageUrlRequests.get(path);const request=(async()=>{const {data,error}=await cloud.storage.from(IMAGE_BUCKET).createSignedUrl(path,3600);if(error)throw error;imageUrls.set(path,{url:data.signedUrl,expires:Date.now()+3500_000});return data.signedUrl;})();imageUrlRequests.set(path,request);try{return await request;}finally{imageUrlRequests.delete(path);}}
async function warmImageUrls(paths){const missing=[...new Set(paths)].filter(path=>typeof path==='string'&&!path.startsWith('data:')&&!imageUrls.has(path)&&!imageUrlRequests.has(path));if(!missing.length)return;const {data,error}=await cloud.storage.from(IMAGE_BUCKET).createSignedUrls(missing,3600);if(error)return;for(const entry of data||[]){if(entry.signedUrl)imageUrls.set(entry.path,{url:entry.signedUrl,expires:Date.now()+3500_000});}}
function prefetchImage(path){if(!path||path.startsWith('data:')||warmedImages.has(path))return;warmedImages.add(path);signedImageUrl(path).then(url=>{const preload=new Image();preload.onload=preload.onerror=()=>warmedImages.delete(path);preload.src=url;}).catch(()=>warmedImages.delete(path));}
function previewPathFor(item,path){return item.imagePreviews?.[path]||path;}
async function createCloudPreview(path){if(previewJobs.has(path)||!cloudUser)return;const ownerId=cloudUser.id;previewJobs.add(path);try{const url=await signedImageUrl(path);const response=await fetch(url);if(!response.ok)return;const original=await response.blob();const compact=await resizeImage(original,1100);if(compact===original||compact.type!=='image/webp'||cloudUser?.id!==ownerId)return;const previewPath=path.replace(/\.[^/.]+$/,'.preview.webp');const {error}=await cloud.storage.from(IMAGE_BUCKET).upload(previewPath,compact,{contentType:'image/webp',upsert:false,cacheControl:'3600'});if(error&&!/already exists|duplicate/i.test(error.message||''))return;if(cloudUser?.id!==ownerId)return;let changed=false;for(const book of state.roadbooks){for(const items of Object.values(book.days||{})){for(const item of items){if(imageList(item).includes(path)&&item.imagePreviews?.[path]!==previewPath){item.imagePreviews={...item.imagePreviews,[path]:previewPath};changed=true;}}}}if(changed)save();}catch{/* Keep showing the original image if preview generation fails. */}finally{previewJobs.delete(path);}}
function displayCloudImage(img,item,index){const images=imageList(item),original=images[index],display=previewPathFor(item,original);setImageElement(img,display);img.addEventListener('load',()=>{if(img.dataset.imageRef!==display)return;const next=images[index+1];if(next)prefetchImage(previewPathFor(item,next));if(display!==original||typeof original!=='string'||original.startsWith('data:'))return;const run=()=>void createCloudPreview(original);if('requestIdleCallback'in window)window.requestIdleCallback(run,{timeout:5000});else setTimeout(run,1000);},{once:true});}
function setImageElement(img,ref){if(!img)return;img.dataset.imageRef=ref;if(ref instanceof File){let url=previewUrls.get(ref);if(!url){url=URL.createObjectURL(ref);previewUrls.set(ref,url);}img.src=url;return;}if(typeof ref!=='string')return;if(ref.startsWith('data:')||ref.startsWith('blob:')){img.src=ref;return;}img.removeAttribute('src');signedImageUrl(ref).then(url=>{if(img.isConnected&&img.dataset.imageRef===ref)img.src=url;}).catch(()=>{if(img.isConnected)img.alt='图片读取失败，请稍后重试';});}
function clearPreviewUrls(){for(const url of previewUrls.values())URL.revokeObjectURL(url);previewUrls.clear();}
...
async function activateCloudUser(user){const email=(user.email||'').trim().toLowerCase();const {data:approval,error:approvalError}=await cloud.from('roadbook_access').select('email').eq('email',email).maybeSingle();if(approvalError||!approval){cloudUser=null;accessNotice=approvalError?'无法确认账号授权，请联系网站所有者。':'此账号尚未获准访问，请联系网站所有者申请邀请。';render();return;}accessNotice='';cloudUser=user;const {data,error}=await cloud.from('roadbook_data').select('payload').eq('user_id',user.id).maybeSingle();if(error){cloudUser=null;accessNotice='读取云端数据失败，请联系网站所有者。';render();return;}if(data?.payload)state=data.payload;const original=structuredClone(state);render();try{if(embeddedImages(state)){await migrateEmbeddedImages();}else if(!data?.payload&&state.roadbooks.length){await persistCloudState(state);}if(!embeddedImages(state)&&clearMatchingLocalCopies(original))showToast('发现不同的本机旧数据，已保留以免丢失');}catch(error){showToast('旧图片迁移失败：'+error.message);}if(authSessionTemporary)showToast('本机空间不足，本次登录只保留到关闭标签页。');}
async function signIn(){ if(!cloud||authBusy)return;const email=$('#cloud-email').value.trim(),password=$('#cloud-password').value,message=$('#cloud-message');if(!email||!password){message.textContent='请输入邮箱和密码。';return;}authBusy=true;message.textContent='正在登录…';try{const {data,error}=await cloud.auth.signInWithPassword({email,password});if(error){message.textContent=`登录失败：${error.message||'请检查账号密码或联系网站所有者。'}`;return;}message.textContent='登录成功，正在核对访问权限…';if(data.session)await activateCloudUser(data.session.user);}catch(error){message.textContent=`登录失败：${error?.message||'网络连接异常，请检查网络后重试。'}`;}finally{authBusy=false;} }
async function requestPasswordSetup(){const email=$('#cloud-email').value.trim(),message=$('#cloud-message');if(!email){message.textContent='请先填写获批邮箱。';return;}message.textContent='正在发送密码设置邮件…';try{const {error}=await cloud.auth.resetPasswordForEmail(email,{redirectTo:window.location.origin+window.location.pathname});if(error){const rateLimited=error.status===429||/rate.?limit|too many requests/i.test(error.message||'');message.textContent=rateLimited?'发送过于频繁：Supabase 邮件服务触发限流，请至少等待 1 分钟后重试；免费默认邮件服务还可能有每小时发送额度限制。':`发送失败：${error.message||'Supabase 未提供具体原因'}`;return;}message.textContent='如果该邮箱已注册，密码设置链接会发送到邮箱，请查收。';}catch(error){message.textContent=`发送失败：${error?.message||'网络连接异常，请检查网络后重试。'}`;}}
...
function renderInviteSetup(){const recovery=passwordLinkType==='recovery';$('#app').innerHTML=`<section class="access-gate"><h1>${recovery?'重设登录密码':'完成账号设置'}</h1><p>${recovery?'请设置新密码，然后使用获批邮箱登录。':'邀请已验证。请设置登录密码，之后可在其他设备使用此邮箱登录。'}</p><div class="field"><label for="invite-password">${recovery?'新密码':'设置密码'}</label><input id="invite-password" type="password" autocomplete="new-password" minlength="6" placeholder="至少 6 位"></div><div class="field"><label for="invite-password-confirm">再次输入密码</label><input id="invite-password-confirm" type="password" autocomplete="new-password" minlength="6" placeholder="确认密码"></div><p id="invite-message" class="hint" aria-live="polite"></p><button class="btn" id="invite-save-password">保存密码并进入</button></section>`;$('#invite-save-password').onclick=setInvitePassword; }
async function signOutCloud(){if(cloud)await cloud.auth.signOut({scope:'local'});cloudUser=null;inviteNeedsPassword=false;history.replaceState({},document.title,window.location.pathname+window.location.search);render();}
...
function renderAccessGate(){const content=!supabaseReady?'<h1>网站暂未开放</h1><p>访问控制服务尚未配置，请稍后联系网站所有者。</p>':`<h1>路书访问</h1><p>此网站仅对获邀账号开放。首次请通过 Supabase 邀请邮件设置密码，然后使用获批邮箱和密码登录。</p>${accessNotice?`<p class="access-notice" role="alert">${esc(accessNotice)}</p>`:''}<div class="field"><label for="cloud-email">获邀邮箱</label><input id="cloud-email" type="email" autocomplete="username" placeholder="name@example.com"></div><div class="field"><label for="cloud-password">密码</label><input id="cloud-password" type="password" autocomplete="current-password" placeholder="账号密码"></div><p id="cloud-message" class="hint" aria-live="polite"></p><button class="btn" id="cloud-signin">邮箱和密码登录</button>`;$('#app').innerHTML=`<section class="access-gate">${content}${cloudUser||accessNotice?'<button class="btn secondary" id="cloud-signout">切换账号</button>':''}</section>`;if($('#cloud-signin'))$('#cloud-signin').onclick=signIn;if($('#cloud-signout'))$('#cloud-signout').onclick=signOutCloud;}
...
function renderEdit(){ const b=book(); if(!b) return renderList(); const items=(b.days[activeDate]||[]).sort((a,c)=>a.startTime.localeCompare(c.startTime)); const editing=items.find(x=>x.id===editingItemId); $('#app').innerHTML=`<header class="topbar"><button type="button" class="icon-btn back" id="back">‹</button><div style="text-align:center"><strong>编辑路书</strong><div class="sub">${esc(b.title||'未命名路书')}</div></div><button type="button" class="icon-btn" id="save" title="保存">✓</button></header><section class="section"><h2>基本信息</h2><div class="form-grid"><div class="field full"><label>路书名称</label><input id="title" value="${esc(b.title)}" placeholder="例如：东京 5 日游" /></div><div class="field"><label>开始日期</label><input id="start" type="date" value="${b.startDate}" /></div><div class="field"><label>结束日期</label><input id="end" type="date" value="${b.endDate}" /></div></div></section><section class="section"><h2>选择日期</h2><div class="day-strip">${dayTabs(b)}</div><p class="hint">${fmtDate(activeDate)} · 已有 ${items.length} 项行程</p></section><section class="section"><h2>${editing?'编辑行程':'新增行程'}</h2>${itemForm(editing)}<div class="actions"><button type="button" class="btn secondary" id="clear-form">取消</button><button type="button" class="btn" id="add-item">${editing?'更新行程':'添加行程'}</button></div></section><section class="section"><h2>当天安排</h2><div class="item-list">${items.length?items.map(editItem).join(''):'<p class="hint">还没有行程，添加第一项吧。</p>'}</div></section>`;
  // Bind the three primary actions immediately, before optional form rendering can fail.
  $('#back').onclick=()=>{screen='view';render();};
  $('#save').onclick=()=>{if(!syncBook())return;try{save();}catch(error){showToast(`保存失败：${error?.message||'请检查设备存储空间'}`);return;}screen='view';showToast('已保存');render();};
  $('#add-item').onclick=addItem;
  renderFormFields();
  clearPreviewUrls();pendingImages=editing?imageList(editing).slice():[];renderImagePreviews();
  pendingTransportPlans=editing?.type==='transport'?transportPlans(editing).slice(1):[]; renderTransportOptions();
  $('#clear-form').onclick=()=>{editingItemId=null;render();};
...
function renderImagePreviews(){const box=$('#image-previews');if(!box)return;box.innerHTML='';const editing=(book()?.days?.[activeDate]||[]).find(item=>item.id===editingItemId);pendingImages.forEach((ref,index)=>{const wrap=document.createElement('div');wrap.className='image-thumb';const img=document.createElement('img');img.alt='行程图片 '+(index+1);const remove=document.createElement('button');remove.type='button';remove.textContent='×';remove.dataset.removeImage=index;remove.onclick=()=>{const removed=pendingImages.splice(index,1)[0];if(removed instanceof File&&previewUrls.has(removed)){URL.revokeObjectURL(previewUrls.get(removed));previewUrls.delete(removed);}renderImagePreviews();};wrap.append(img,remove);box.append(wrap);setImageElement(img,editing&&typeof ref==='string'?previewPathFor(editing,ref):ref);});}
function addImages(e){const files=Array.from(e.target.files||[]).slice(0,9-pendingImages.length);if(!files.length)return;pendingImages.push(...files);renderImagePreviews();e.target.value='';}
...
async function addItem(){
  if(imageSaveBusy||!syncBook())return;
  const startTime=$('#startTime').value,endTime=$('#endTime').value,type=$('#type').value;
  if(!startTime||!endTime||endTime<=startTime)return showToast('结束时间需晚于开始时间');
  const prior=(book().days[activeDate]||[]).find(x=>x.id===editingItemId);
  let data={id:editingItemId||uid(),startTime,endTime,type,status:prior?.status||'pending',images:[],imagePreviews:{}};
  if(type==='transport'){
    const primary={mode:$('#mode').value,origin:$('#origin').value.trim(),destination:$('#destination').value.trim()};
    const plans=[primary,...readAlternativePlans()];
    if(plans.some(plan=>!plan.origin||!plan.destination))return showToast('请填写每个交通方案的出发与到达地点');
    data={...data,...primary,plans};
  }
  if(type==='activity'){
    data={...data,name:$('#name').value.trim(),location:$('#location').value.trim()};
    if(!data.name||!data.location)return showToast('请填写项目名称与地点');
  }
  if(type==='meal'){
    data={...data,cuisine:$('#cuisine').value.trim(),feature:$('#feature').value.trim(),location:$('#location').value.trim()};
    if(!data.location)return showToast('请填写餐厅地点');
  }
  const button=$('#add-item');imageSaveBusy=true;button.disabled=true;button.textContent='正在上传并保存…';
  try{
    for(const image of pendingImages)data.images.push(image instanceof File?await uploadImage(image):image);
    for(const path of data.images)if(prior?.imagePreviews?.[path])data.imagePreviews[path]=prior.imagePreviews[path];
    const previous=book().days[activeDate]||[],arr=previous.slice(),n=arr.findIndex(x=>x.id===data.id);
    if(n>=0)arr[n]=data;else arr.push(data);
    book().days[activeDate]=arr;
    try{if(cloudUser)await persistCloudState(state);else save();}catch(error){book().days[activeDate]=previous;throw error;}
    editingItemId=null;clearPreviewUrls();showToast(n>=0?'行程已更新':'行程已添加');render();
  }catch(error){showToast(`保存失败：${error.message||'请重试'}`);}
  finally{imageSaveBusy=false;if(button.isConnected){button.disabled=false;button.textContent=editingItemId?'更新行程':'添加行程';}}
}
...
function renderComplexView(){
  ...
  const item=items[index]; activeItemId=item.id; activeDate=item.date; const images=imageList(item);
  const visual=images.length ? '<div class="complex-photo"><img alt="行程图片"><span class="photo-count">1 / '+images.length+'</span></div>' : '<div class="complex-placeholder '+item.type+'"><span>'+({transport:'✈',activity:'✦',meal:'⌁'}[item.type])+'</span></div>';
  ...
  if(images.length){displayCloudImage($('.complex-photo img'),item,0);void warmImageUrls(images.slice(1).map(path=>previewPathFor(item,path))).catch(()=>{});}
  ...
  let photo=0; document.querySelectorAll('[data-photo]').forEach(dot=>dot.onclick=()=>{photo=Number(dot.dataset.photo);displayCloudImage($('.complex-photo img'),item,photo);$('.photo-count').textContent=(photo+1)+' / '+images.length;document.querySelectorAll('[data-photo]').forEach(x=>x.classList.toggle('active',Number(x.dataset.photo)===photo));});
  const photoBox=$('.complex-photo'); if(photoBox&&images.length>1){let x=0;photoBox.addEventListener('touchstart',e=>x=e.touches[0].clientX,{passive:true});photoBox.addEventListener('touchend',e=>{e.stopPropagation();const dx=e.changedTouches[0].clientX-x;if(Math.abs(dx)<35)return;photo=(photo+(dx<0?1:-1)+images.length)%images.length;displayCloudImage(photoBox.querySelector('img'),item,photo);photoBox.querySelector('.photo-count').textContent=(photo+1)+' / '+images.length;document.querySelectorAll('[data-photo]').forEach(z=>z.classList.toggle('active',Number(z.dataset.photo)===photo));},{passive:true});}
}
