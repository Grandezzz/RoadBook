...
为减少手机流量和等待时间，新上传的 JPEG/PNG/WebP 会在浏览器内存中缩至最长边 1600 像素，并在更小的 WebP 文件可用时上传。已有云端大图首次打开后，会在后台生成最长边 1100 像素的私有预览图供后续查看；原图保留在云端，首次处理仍需读取原图。相邻图片的访问链接和图片内容会提前加载到浏览器的临时网络缓存，不会写入应用的 localStorage 图片缓存。
